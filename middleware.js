...
export default async function middleware() {
  try {
    ...
    const calendarStart = html.indexOf('function ClayContentCalendar({ contentList, onSelectContent, onCreateNew }) {');
    const calendarEnd = html.indexOf('function ClayContentTable', calendarStart);

    if (calendarStart !== -1 && calendarEnd !== -1) {
      let calendar = html.slice(calendarStart, calendarEnd);
      ...
      calendar = calendar.replace(
        ...
        `<div className="space-y-5">\n          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">\n            <div className="flex items-center gap-2">\n              <button type="button" onClick={() => { const d = new Date(year, month - 2, 1); setFilterMonth(d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0')); }} className="clay-btn bg-white px-3 py-2 text-slate-600 text-xs font-bold">←</button>\n              <button type="button" onClick={() => { const d = new Date(year, month, 1); setFilterMonth(d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0')); }} className="clay-btn bg-white px-3 py-2 text-slate-600 text-xs font-bold">→</button>\n              <button type="button" onClick={() => { const d = new Date(); setFilterMonth(d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0')); }} className="clay-btn bg-white px-3 py-2 text-xs font-bold text-indigo-600">Today</button>\n              <div className="ml-2 text-base font-extrabold text-slate-900">{new Date(year, month - 1, 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</div>\n            </div>\n            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Month View · Shared Team Calendar</div>\n          </div>`
      );
      calendar = calendar.replace(
    ...
    // Pass month setter into the calendar toolbar.
    html = html.replace(
